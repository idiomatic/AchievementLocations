-- this file is machine generated.  do not edit.

local AL = LibStub:GetLibrary("AchievementLocations-1.0")
local function A(row) AL:AddLocation(row) end

-- Reputation: 60 Exalted Reputations
A{"AhnQiraj", 6742, criterion=982, faction=910} -- Brood of Nozdormu
A{"AhnQiraj", 6742, criterion=982, faction=609} -- Cenarion Circle
A{"AlteracValley", 6742, criterion=982, faction=730, side="alliance"} -- Stormpike Guard
A{"AlteracValley", 6742, criterion=982, faction=729, side="horde"} -- Frostwolf Clan
A{"Arathi", 6742, criterion=982, faction=70} -- Syndicate
A{"ArathiBasin", 6742, criterion=982, faction=509, side="alliance"} -- The League of Arathor
A{"ArathiBasin", 6742, criterion=982, faction=510, side="horde"} -- The Defilers
A{"Ashran", 6742, criterion=982, faction=1681, side="horde"} -- Vol'jin's Spear
A{"Ashran", 6742, criterion=982, faction=1682, side="alliance"} -- Wrynn's Vanguard
A{"AuchenaiCrypts", 6742, criterion=982, faction=1011} -- Lower City
A{"Barrens", 6742, criterion=982, faction=369, note="Fray Island"} -- Gadgetzan
A{"BlackTemple", 6742, criterion=982, faction=1012} -- Ashtongue Deathsworn

-- Reputation/The Burning Crusade: A Quest a Day Keeps the Ogres at Bay
A{"BladesEdgeMountains", 896, 0.2850, 0.5750, faction=1038}

-- Reputation: 60 Exalted Reputations
A{"BladesEdgeMountains", 6742, 0.2850, 0.5750, criterion=982, faction=1038} -- Ogri'la
A{"BladesEdgeMountains", 6742, 0.2780, 0.5270, criterion=982, faction=1031} -- Sha'tari Skyguard
A{"BoreanTundra", 6742, criterion=982, faction=1064, side="horde"} -- The Taunka
A{"CoTHillsbradFoothills", 6742, criterion=982, faction=989} -- Keepers of Time
A{"CoTTheBlackMorass", 6742, 0.4800, 0.7450, criterion=982, faction=989} -- Keepers of Time

-- Reputation/Wrath of the Lich King: The Kirin Tor
A{"Dalaran", 1008, faction=1090}

-- Reputation/Wrath of the Lich King: Northrend Vanguard
A{"Dalaran", 1010, criterion=4742, faction=1090} -- The Kirin Tor

-- Feats of Strength: Insane in the Membrane
A{"DarkmoonFaireIsland", 2336, quest=909} -- Darkmoon Faire

-- Reputation: 60 Exalted Reputations
A{"DarkmoonFaireIsland", 6742, criterion=982, faction=909} -- Darkmoon Faire
A{"Deepholm", 6742, criterion=982, faction=1171} -- Therazane
A{"Desolace", 6742, criterion=982, faction=92} -- Gelkis Clan Centaur
A{"Desolace", 6742, criterion=982, faction=93} -- Magram Clan Centaur

-- Reputation/Wrath of the Lich King: Tuskarrmageddon
A{"Dragonblight", 949, faction=1073}

-- Reputation/Wrath of the Lich King: The Wyrmrest Accord
A{"Dragonblight", 1007, faction=1091}

-- Reputation/Wrath of the Lich King: Northrend Vanguard
A{"Dragonblight", 1010, criterion=4741, faction=1091} -- The Wyrmrest Accord

-- Reputation: 60 Exalted Reputations
A{"Dragonblight", 6742, criterion=982, faction=1091} -- The Wyrmrest Accord

-- Reputation: The Argent Champion
A{"EasternPlaguelands", 945, criterion=4737, faction=1106} -- The Argent Crusade
A{"EasternPlaguelands", 945, criterion=2020, faction=529} -- The Argent Dawn

-- Reputation/Classic: The Argent Dawn
A{"EasternPlaguelands", 946, faction=529}

-- Reputation: 60 Exalted Reputations
A{"EasternPlaguelands", 6742, criterion=982, faction=529} -- Argent Dawn

-- Reputation: The Diplomat
A{"Felwood", 942, criterion=2011, faction=576, side="alliance"} -- Exalted with Timbermaw Hold

-- Reputation: The Diplomat
A{"Felwood", 943, criterion=2011, faction=576, side="horde"} -- Exalted with Timbermaw Hold

-- Reputation/Classic: They Love Me In That Tunnel
A{"Felwood", 944, faction=576}

-- Reputation: 60 Exalted Reputations
A{"Felwood", 6742, criterion=982, faction=576} -- Timbermaw Hold
A{"Firelands", 6742, criterion=982, faction=1204} -- Avengers of Hyjal
A{"FrostfireRidge", 6742, criterion=982, faction=1445, side="horde"} -- Frostwolf Orcs
A{"Ghostlands", 6742, criterion=982, faction=922, side="horde"} -- Tranquillien
A{"Gorgrond", 6742, criterion=982, faction=1708, side="horde"} -- Laughing Skull Orcs
A{"HellfireRamparts", 6742, criterion=982, faction=946, side="alliance"} -- Honor Hold
A{"HellfireRamparts", 6742, criterion=982, faction=947, side="horde"} -- Thrallmar

-- Feats of Strength: Insane in the Membrane
A{"HillsbradFoothills", 2336, quest=349} -- Ravenholdt

-- Reputation: 60 Exalted Reputations
A{"HillsbradFoothills", 6742, criterion=982, faction=349, note="may need a level 60 rogue for pickpocketing Heavy Junkbox"} -- Ravenholdt
A{"HowlingFjord", 6742, 0.3080, 0.2860, criterion=982, faction=1068, side="alliance"} -- Explorer's League
A{"HowlingFjord", 6742, criterion=982, faction=1067, side="horde"} -- The Hand of Vengeance

-- Reputation/The Burning Crusade: The Scale of the Sands
A{"Hyjal", 959, faction=990}

-- Reputation/The Burning Crusade: The Violet Eye
A{"Hyjal", 960, faction=967}

-- Reputation: 60 Exalted Reputations
A{"Hyjal", 6742, criterion=982, faction=990} -- The Scale of Sands
A{"Hyjal", 6742, criterion=982, faction=1158} -- Guardians of Hyjal
A{"IcecrownCitadel", 6742, criterion=982, faction=1156} -- The Ashen Verdict

-- Reputation/Wrath of the Lich King: Knights of the Ebon Blade
A{"IcecrownGlacier", 1009, faction=1098}

-- Reputation/Wrath of the Lich King: Northrend Vanguard
A{"IcecrownGlacier", 1010, criterion=4743, faction=1098} -- Knights of the Ebon Blade
A{"IcecrownGlacier", 1010, criterion=4737, faction=1106} -- The Argent Crusade

-- World Events/Argent Tournament: Exalted Champion of Darnassus
A{"IcecrownGlacier", 2760, 0.7630, 0.1910, criterion=5328, faction=69, side="alliance"} -- Exalted with Darnassus

-- World Events/Argent Tournament: Exalted Champion of the Exodar
A{"IcecrownGlacier", 2761, 0.7610, 0.1910, criterion=5332, faction=930, side="alliance"} -- Exalted with Exodar

-- World Events/Argent Tournament: Exalted Champion of Gnomeregan
A{"IcecrownGlacier", 2762, 0.7650, 0.1980, criterion=5329, faction=54, side="alliance"} -- Exalted with Gnomeregan Exiles

-- World Events/Argent Tournament: Exalted Champion of Ironforge
A{"IcecrownGlacier", 2763, 0.7660, 0.1950, criterion=5330, faction=47, side="alliance"} -- Exalted with Ironforge

-- World Events/Argent Tournament: Exalted Champion of Stormwind
A{"IcecrownGlacier", 2764, 0.7660, 0.1920, criterion=5331, faction=72, side="alliance"} -- Exalted with Stormwind

-- World Events/Argent Tournament: Exalted Champion of Orgrimmar
A{"IcecrownGlacier", 2765, criterion=5314, faction=76, side="horde"} -- Exalted with Orgrimmar

-- World Events/Argent Tournament: Exalted Champion of Sen'jin
A{"IcecrownGlacier", 2766, criterion=5313, faction=530, side="horde"} -- Exalted with Darkspear Trolls

-- World Events/Argent Tournament: Exalted Champion of Silvermoon City
A{"IcecrownGlacier", 2767, criterion=5317, faction=911, side="horde"} -- Exalted with Silvermoon City

-- World Events/Argent Tournament: Exalted Champion of Thunder Bluff
A{"IcecrownGlacier", 2768, criterion=5315, faction=81, side="horde"} -- Exalted with Thunder Bluff

-- World Events/Argent Tournament: Exalted Champion of the Undercity
A{"IcecrownGlacier", 2769, criterion=5316, faction=68, side="horde"} -- Exalted with Undercity

-- Reputation: 60 Exalted Reputations
A{"IcecrownGlacier", 6742, criterion=982, faction=1098} -- Knights of the Ebon Blade
A{"IsleoftheThunderKing", 6742, criterion=982, faction=1387, side="alliance"} -- Kirin Tor Offensive
A{"IsleoftheThunderKing", 6742, criterion=982, faction=1388, side="horde"} -- Sunreaver Onslaught
A{"Karazhan", 6742, criterion=982, faction=967} -- The Violet Eye
A{"ManaTombs", 6742, criterion=982, faction=933} -- The Consortium
A{"MoltenCore", 6742, criterion=982, faction=749} -- Hydraxian Waterlords

-- Reputation/The Burning Crusade: Oh My, Kurenai
A{"Nagrand", 899, 0.7400, 0.6700, faction=978, side="alliance"}

-- Reputation/The Burning Crusade: Mag'har of Draenor
A{"Nagrand", 901, faction=941, side="horde"}

-- Reputation: 60 Exalted Reputations
A{"Nagrand", 6742, 0.7400, 0.6700, criterion=982, faction=978, side="alliance"} -- Kurenai
A{"Nagrand", 6742, criterion=982, faction=941, side="horde"} -- The Mag'har
A{"NagrandDraenor", 6742, criterion=982, faction=1711} -- Steamwheedle Preservation Society

-- Reputation/The Burning Crusade: Chief Exalted Officer
A{"Netherstorm", 902, faction=933}

-- Reputation: 60 Exalted Reputations
A{"SearingGorge", 6742, criterion=982, faction=59} -- Thorium Brotherhood
A{"SethekkHalls", 6742, criterion=982, faction=1011} -- Lower City

-- Reputation/The Burning Crusade: On Wings of Nether
A{"ShadowmoonValley", 898, 0.6600, 0.8600, faction=1015}

-- Reputation: 60 Exalted Reputations
A{"ShadowmoonValley", 6742, 0.6600, 0.8300, criterion=982, faction=1015} -- Netherwing
A{"ShadowmoonValleyDR", 6742, criterion=982, faction=1731, side="alliance"} -- Council of Exarchs
A{"ShattrathCity", 6742, criterion=982, faction=932} -- The Aldor
A{"ShattrathCity", 6742, criterion=982, faction=934} -- The Scryers

-- Reputation/Wrath of the Lich King: Frenzyheart Tribe
A{"SholazarBasin", 950, faction=1104}

-- Reputation/Wrath of the Lich King: The Oracles
A{"SholazarBasin", 951, faction=1105}

-- Reputation/Wrath of the Lich King: Mercenary of Sholazar
A{"SholazarBasin", 952, criterion=2040} -- Frenzyheart Tribe
A{"SholazarBasin", 952, criterion=2039} -- The Oracles

-- Reputation: 60 Exalted Reputations
A{"SholazarBasin", 6742, criterion=982, faction=1104} -- The Frenzyheart
A{"SholazarBasin", 6742, criterion=982, faction=1105} -- The Oracles
A{"SpiresOfArak", 6742, criterion=982, faction=1515} -- Arakkoa Outcasts

-- Reputation/The Burning Crusade: You're So Offensive
A{"Sunwell", 897, faction=1077}

-- Reputation: 60 Exalted Reputations
A{"Sunwell", 6742, criterion=982, faction=1077} -- Shattered Sun Offensive
A{"Talador", 6742, criterion=982, faction=1710, side="alliance"} -- Sha'tari Defense

-- Reputation/Draenor: Tanaan Diplomat
A{"TanaanJungle", 10349, criterion=28978, faction=1848, side="horde"} -- Vol'jin's Headhunters
A{"TanaanJungle", 10349, criterion=28979, faction=1850, side="horde"} -- The Saberstalkers
A{"TanaanJungle", 10349, criterion=28980, faction=1849, side="horde"} -- Order of the Awakened

-- Reputation/Draenor: Tanaan Diplomat
A{"TanaanJungle", 10350, criterion=28981, faction=1847, side="alliance"} -- Hand of the Prophet
A{"TanaanJungle", 10350, criterion=28980, faction=1849, side="alliance"} -- Order of the Awakened
A{"TanaanJungle", 10350, criterion=28979, faction=1850, side="alliance"} -- The Saberstalkers

-- Reputation: 60 Exalted Reputations
A{"Tanaris", 6742, criterion=982, faction=21, note="Lost Rigger Cove"} -- Booty Bay
A{"Tanaris", 6742, criterion=982, faction=577} -- Everlook
A{"Tanaris", 6742, criterion=982, faction=470} -- Ratchet

-- Reputation/The Burning Crusade: Flying High Over Skettis
A{"TerokkarForest", 894, faction=1031}

-- Reputation: 60 Exalted Reputations
A{"TerokkarForest", 6742, 0.6450, 0.6640, criterion=982, faction=1031} -- Sha'tari Skyguard
A{"TheArcatraz", 6742, criterion=982, faction=935} -- The Sha'tar
A{"TheBloodFurnace", 6742, criterion=982, faction=946, side="alliance"} -- Honor Hold
A{"TheBloodFurnace", 6742, criterion=982, faction=947, side="horde"} -- Thrallmar
A{"TheBotanica", 6742, criterion=982, faction=935} -- The Sha'tar

-- Feats of Strength: Avast Ye, Admiral!
A{"TheCapeOfStranglethorn", 871, 0.4660, 0.9520}

-- Feats of Strength: Insane in the Membrane
A{"TheCapeOfStranglethorn", 2336, quest=87} -- Bloodsail Buckaneers

-- Reputation: 60 Exalted Reputations
A{"TheCapeOfStranglethorn", 6742, criterion=982, faction=87} -- Bloodsail Buccaneers
A{"TheMechanar", 6742, criterion=982, faction=935} -- The Sha'tar
A{"TheShatteredHalls", 6742, criterion=982, faction=946, side="alliance"} -- Honor Hold
A{"TheShatteredHalls", 6742, criterion=982, faction=947, side="horde"} -- Thrallmar
A{"TheSlavePens", 6742, criterion=982, faction=942} -- Cenarion Expedition
A{"TheSteamvault", 6742, criterion=982, faction=942} -- Cenarion Expedition
A{"TheStormPeaks", 6742, criterion=982, faction=1094, side="alliance"} -- The Silver Covenant
A{"TheStormPeaks", 6742, criterion=982, faction=1119} -- The Sons of Hodir
A{"TheUnderbog", 6742, criterion=982, faction=942} -- Cenarion Expedition
A{"TheUnderbog", 6742, criterion=982, faction=970} -- Sporeggar
A{"TolBaradDailyArea", 6742, criterion=982, faction=1177, side="alliance"} -- Baradin's Wardens
A{"TolBaradDailyArea", 6742, criterion=982, faction=1178, side="horde"} -- Hellscream's Reach
A{"TwilightHighlands", 6742, criterion=982, faction=1172, side="horde"} -- Dragonmaw Clan
A{"TwilightHighlands", 6742, criterion=982, faction=1174, side="alliance"} -- Wildhammer Clan
A{"Uldum", 6742, criterion=982, faction=1173} -- Ramkahen

-- Reputation/Pandaria: Friend on the Farm
A{"ValleyoftheFourWinds", 6551, criterion=19396, faction=1277} -- Chee Chee
A{"ValleyoftheFourWinds", 6551, criterion=19397, faction=1275} -- Ella
A{"ValleyoftheFourWinds", 6551, criterion=19398, faction=1283} -- Farmer Fung
A{"ValleyoftheFourWinds", 6551, criterion=19399, faction=1282} -- Fish Fellreed
A{"ValleyoftheFourWinds", 6551, criterion=19400, faction=1281} -- Gina Mudclaw
A{"ValleyoftheFourWinds", 6551, criterion=19401, faction=1279} -- Haohan Mudclaw
A{"ValleyoftheFourWinds", 6551, criterion=19402, faction=1273} -- Jogu the Drunk
A{"ValleyoftheFourWinds", 6551, criterion=19403, faction=1276} -- Old Hillpaw
A{"ValleyoftheFourWinds", 6551, criterion=19404, faction=1278} -- Sho
A{"ValleyoftheFourWinds", 6551, criterion=19405, faction=1280} -- Tina Mudclaw

-- Reputation: 60 Exalted Reputations
A{"Vashjir", 6742, criterion=982, faction=1135} -- The Earthen Ring
A{"VioletHold", 6742, criterion=982, faction=1126, side="alliance"} -- The Frostborn
A{"WarsongGulch", 6742, criterion=982, faction=890, side="alliance"} -- Silverwing Sentinels
A{"WarsongGulch", 6742, criterion=982, faction=889, side="horde"} -- Warsong Outriders
A{"Winterspring", 6742, criterion=982, faction=589} -- Wintersaber Trainers

-- Reputation/The Burning Crusade: Cenarion War Hippogryph
A{"Zangarmarsh", 893, 0.7920, 0.6380}

-- Reputation/The Burning Crusade: The Czar of Sporeggar
A{"Zangarmarsh", 900, faction=970}

-- Reputation: The Diplomat
A{"Zangarmarsh", 942, criterion=4757, faction=970, side="alliance"} -- Exalted with the Sporeggar

-- Reputation: The Diplomat
A{"Zangarmarsh", 943, criterion=4757, faction=970, side="horde"} -- Exalted with the Sporeggar
